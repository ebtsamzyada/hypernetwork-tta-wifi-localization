...
The 4 held-out test sessions turn out to have OVERLAPPING timestamp
ranges (confirmed: they were captured by different users/phones on the
same day, apparently concurrently) -- sorting all rows by raw global
TIMESTAMP therefore interleaves physically unrelated sessions
sample-by-sample (704 session transitions across a 2750-row "sorted"
timeline!), which breaks the premise of "one continuous deployment at one
site" entirely. Each session is treated as its OWN independent
deployment: sessions are ordered by their own earliest timestamp and
concatenated as CONTIGUOUS blocks (never interleaved), and drift
(accumulated random-walk offset + dropout schedule progress) resets at
the start of every session block, since a new session means a new
physical site starting a fresh deployment.
"""
...
RSSI_FLOOR = -104.0  # weakest real detected RSSI in the dataset
RSSI_CEIL = 0.0
