"""
Step 2 (SOD-HCXY variant): Train the frozen base MLP and report an honest
baseline MAE.

Simpler than the UJIIndoorLoc variant: SOD-HCXY is single-floor (FloorID
is constant), so there's no floor-conflation issue and no floor
classifier is needed -- just a plain BaseMLP mapping RSSI straight to
(x, y).
...
def main():
    ...
    model = BaseMLP(input_dim=X_tr.shape[1])
