...
SEED = 42
BUFFER_SIZE = 50  # raised from 30: most test sessions (90-190 rows) and all
# train sessions (720+ rows) can support a much less noisy buffer; only
# the shortest test session (40 rows) loses the ability to ever adapt,
# which is an acceptable trade for materially cleaner reference/current
# statistics on the other ~95% of test data.
NUM_META_STEPS = 1500
...
def load_frozen_base(out_dir=OUT_DIR):
    """Loads the frozen graph/set-attention base model (GraphBaseModel).
    `head_in` for the hypernetwork is still 128 (the encoder's out_dim),
    so the hypernetwork architecture itself is completely unchanged --
    only the frozen base network's trunk differs from the plain-MLP
    pipeline in src_sod/."""
    ckpt = torch.load(f"{out_dir}/base_model.pt", weights_only=False)
    base_model = GraphBaseModel(num_aps=ckpt["input_dim"])
